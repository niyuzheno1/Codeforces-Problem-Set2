...
fun substr(a : String, b : String) : Boolean{
...
fun solve() {
  ...
  val b = a.sortedWith(Comparator<Int>{x,y ->
    sar[x].length.compareTo(sar[y].length)})
  var good = true; 
 for( i in 0..(n-2)){
    if(sar[b[i]].length == sar[b[i+1]].length){
        if(sar[b[i]].compareTo(sar[b[i+1]]) != 0){
            good = false
            break
        }
    }else{
        if(substr(sar[b[i]],sar[b[i+1]]) == false){
            good = false
            break
        }
    }
