...
class dsu{
    ...
    fun merge(x : Long, y : Long) {
        var fx = F(x)
        var fy = F(y)
        f[fx.toInt()] = f[fy.toInt()]
        F(fx)
    }
...
fun solve() {
  ...
  for(i in 1..m){
   ...
   val B = b.sortedWith(Comparator<LongArray>{x,y ->
    x[2].compareTo(y[2])})
   m = B.size
   var ans : Long = 0.toLong()
   for(i in 0..(m-1)){
    if(d.F(B[i][0]) != d.F(B[i][1])){
        ans += B[i][2]
        d.merge(B[i][0],B[i][1]);
    }
   }
