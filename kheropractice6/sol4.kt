...
fun okay(x : Long, n : Long, s : Long): Int{
    var days = 0.toLong()
    var acsum = 0.toLong()
    var xx = x
    while(xx > 1.toLong() && days < n){
        acsum = acsum + xx
        xx = (xx+1.toLong())/2.toLong()
        days = days + 1.toLong()
    }
    acsum = acsum + n-days
    if(acsum <= s){
...
fun solve() {
    ...
    var low = max(l-1.toLong(), 0.toLong())
    var upper = min(r+((1).toLong()), s+(1).toLong())
    for(i in (low)..(upper)){
        if(okay(i,n,s) < 0){
            println(i-1.toLong());
            break
